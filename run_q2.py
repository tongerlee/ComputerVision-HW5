...
# Jiatong: Moved Q2.5 before Q2.4
# Save the old params
import copy
    ...
    if 'W' in k:
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                ...
                params_plus[k][i, j] += eps
                h1 = forward(x, params_plus, 'layer1')
                probs = forward(h1, params_plus, 'output', softmax)
                loss_plus, acc_plus = compute_loss_and_acc(y, probs)

                params_minus = copy.deepcopy(params)
                params_minus[k][i, j] -= eps
                h1 = forward(x, params_minus, 'layer1')
                probs = forward(h1, params_minus, 'output', softmax)
                loss_minus, acc_minus = compute_loss_and_acc(y, probs)

                params['grad_'+ k][i, j] = (loss_plus - loss_minus) / (2.0 * eps)
    else:
        for i in range(v.shape[0]):
            params_plus = copy.deepcopy(params)
            params_plus[k][i] += eps
            ...
            params['grad_'+k][i] = (loss_plus - loss_minus) / (2.0 * eps)
